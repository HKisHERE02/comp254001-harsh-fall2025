...
public class CircularlyLinkedList<E> implements Cloneable{
    ...
    public CircularlyLinkedList<E> clone() throws CloneNotSupportedException {
        CircularlyLinkedList<E> other = (CircularlyLinkedList<E>) super.clone(); // shallow copy
        if (size > 0) {
            // create a new node for head
            Node<E> originalHead = tail.getNext();
            Node<E> copyHead = new Node<>(originalHead.getElement(), null);
            Node<E> walkOriginal = originalHead.getNext();
            Node<E> walkCopy = copyHead;

            // copy all nodes
            while (walkOriginal != originalHead) {
                Node<E> newest = new Node<>(walkOriginal.getElement(), null);
                walkCopy.setNext(newest);
                walkCopy = newest;
                walkOriginal = walkOriginal.getNext();
            }

            // close the circular link
            walkCopy.setNext(copyHead);
