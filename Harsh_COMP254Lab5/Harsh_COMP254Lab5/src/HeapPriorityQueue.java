...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
    ...
    /** Recursively moves the entry at index j higher to restore the heap property. */
    protected void upheap(int j) {
        if (j == 0) return;  //root
        int p = parent(j);
        if (compare(heap.get(j), heap.get(p)) < 0) {  //heap property not satisfied
            swap(j, p);
            upheap(p);  //recursively fix at parent
        }
    }
